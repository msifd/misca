...
public class BlessCommand extends CommandBase  {
    ...
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos targetPos) {
        switch (args.length) {
            ...
            case 2:
                return getListOfStringsMatchingLastWord(args, potionArg, enchantArg, listArg);
            case 3:
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        switch (args[1].toLowerCase()) {
            case potionArg:
                if (args.length < 3) throw new SyntaxErrorException("Usage: " + getUsage(sender));
                addPotion(sender, player, args[2], parseInt(args[3]));
                break;
            case enchantArg:
                if (args.length < 3) throw new SyntaxErrorException("Usage: " + getUsage(sender));
                addEnchant(sender, player, args[2], parseInt(args[3]));
                break;
            case listArg:
