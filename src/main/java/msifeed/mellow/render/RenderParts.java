...
//    }
    ...
    public static void string(String text, Geom geom, TextPref pref) {
        final FontRenderer fr = Minecraft.getMinecraft().fontRenderer;

        GL11.glPushMatrix();
        GL11.glTranslatef(0, 0, geom.z);

        fr.drawString(text, geom.x + pref.xOff, geom.y + pref.yOff, pref.color);

        GL11.glColor4f(1, 1, 1, 1);
        GL11.glPopMatrix();
    }

    public static void lines(Collection<String> lines, Geom geom, TextPref pref) {
        final FontRenderer fr = Minecraft.getMinecraft().fontRenderer;
